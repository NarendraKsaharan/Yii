...
class ProductController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index', 'create', 'update', 'delete', 'view', 'detail', 'cart'],
                        'allow' => true,
                        'roles' => ['@'],
    ...
    public function actionCart()
    {
        ...
        $quoteId = $session->has('quoteId'); 
        echo "<pre>";
        print_r($session->has('quoteId'));
        exit;
        if ($session->has('quote_id')) {
            ...
            // print_r($quoteId);
            // exit;
            $cart = Cart::find()->where(['quote_id' => $quoteId])->one();
            // if(empty($cart)){
            //     $cart = new Cart();
            // } 
            // echo '<pre>'; print_r($quoteId); echo '</pre>';
            ...
            $cart = new Cart();
            $quoteId        = Yii::$app->security->generateRandomString(20); 
            $cart->quote_id = $quoteId;
            // $cart->user_id = Yii::$app->user->id; 
            $cart->save();
            $cartItem             = new CartItem();
            ...
            Yii::$app->session->set('quote_id',$quoteId);
            $quoteId = $session->has('quote_id');

            // echo "<pre>";
            // print_r(Yii::$app->session->set('quote_id',$quoteId));
            // exit;
